...
var createItemFor = function (orderId) {
    $.post('/Customer/Items/Create/', { orderId: orderId }, function (data) {
        $('#mainModal').html(data);
        $('#mainModal').modal();
        refreshValidation();
        setupItemCreateForm();
...
var editOrder = function (orderId) {
    $.post('/Customer/Orders/Edit/', { id: orderId }, function (data) {
        $('#mainModal').html(data);
        refreshValidation();
        $('.editor-field').keypress(function (e) {
            if (e.which == 13) {
...
var cancelItem = function (itemId) {
    bootbox.confirm("Are you sure to cancel/delete this item?", function (result) {
        if (result) {
            $.post('/Customer/Items/Cancel/' + itemId, function (result) {
                if (result.success) {
                    $.blockUI();
                    window.location.reload();
                } else {
...
var editItem = function (itemId) {
    $.post('/Customer/Items/Edit/', { id: itemId }, function (data) {
        $('#mainModal').html(data);
        refreshValidation();
        $('.editor-field').keypress(function (e) {
            if (e.which == 13) {
