...
namespace HDMS.Models.Tsp
{
    ...
    class Tsp
    {
        ...
        public List<int> Begin(Cities cityList)
        {
            int populationSize = 10000;
            int maxGenerations = 150000;
            int groupSize = 7;
            int mutation = 3;
            int seed = 0;
            int chanceToUseCloseCity = 90;
            ...
            var orderedCityIdList = new List<int>();
            orderedCityIdList.Add(cityList[0].Id);
            var link = population.BestTour[0];
            while (orderedCityIdList.Count != population.BestTour.Count)
            {
                if (!orderedCityIdList.Contains(cityList[link.Connection2].Id))
                {
                    orderedCityIdList.Add(cityList[link.Connection2].Id);
                    link = population.BestTour[link.Connection2];
                }
                else
                {
                    orderedCityIdList.Add(cityList[link.Connection1].Id);
                    link = population.BestTour[link.Connection1];
                }
            }
            orderedCityIdList.Remove(orderedCityIdList[0]);
            return orderedCityIdList;
        }
